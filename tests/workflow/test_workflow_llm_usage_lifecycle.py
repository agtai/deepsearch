...
@pytest.mark.asyncio
async def test_run_resets_and_pops_workflow_llm_usage_when_stats_enabled():
    """验证 run 在开启统计时会清理 workflow 级 token 汇总。"""
    ...
    with patch.object(agent, "_initialize_tools", side_effect=_fake_initialize_tools), patch(
        ...
        "openjiuwen_deepsearch.framework.openjiuwen.agent.workflow.reset_workflow_llm_usage",
    ) as mock_reset_usage, patch(
    ...
    assert isinstance(error_content.get("exception_info"), str)
    assert error_content["exception_info"] != ""
    assert error_content["workflow_llm_token_usage"] == fake_usage

    mock_reset_usage.assert_called_once_with(conversation_id)
